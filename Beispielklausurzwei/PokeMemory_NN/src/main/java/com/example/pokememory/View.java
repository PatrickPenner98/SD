    ...
    public Background getBackground(int i, int j)
    {
        ...
        try {
            if (model.istAufgedeckt(i, j)) {
                ...
                image = new Image(getClass().getResource("pokemon" + pokeIndex + ".png").toURI().toString(), View.GROESSE_FELD, View.GROESSE_FELD, true, true);
            } else {
                image = new Image(this.getClass().getResource("pokeball.png").toURI().toString(), View.GROESSE_FELD, View.GROESSE_FELD, true, true);
            }
        }
        catch (Exception e) {}
